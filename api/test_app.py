...
def test_print_db_executes(test_client, capsys):
    ...
    # add camtured console print to a variable
    ...
    if contains_hotel or contains_no_data_message:
        assert True
    # else:
    #     assert False, "print_db output is not as expected"
...
def test_api_key_collection_post(test_client, username, password):
    ...
    assert response.status_code in [200, 401], f"Unexpected status code returned: {response.status_code}"
...
def test_api_key_collection_delete(test_client, api_credentials):
    ...
    assert response.status_code == 200, "Failed to delete API key"
...
# test GET cutomer info without username
...
# test GET cutomer info without apikey
@pytest.mark.parametrize("customer_id", [1])
...
#test updating cutomer info and test if email already in use
...
def test_customer_item_put(test_client, api_credentials, customer_id, update_data):
    ...
    assert response.status_code in [200, 409, 404, 403], f"Unexpected status code returned: {response.status_code}"


#test updating cutomer info without username
@pytest.mark.parametrize("customer_id, update_data", [
    (1, {"name": "Jane Updated", "mail": "jane.updated@example.com", "phone": "987654321", "address": "123 Updated Street"}),
])
...
def test_room_collection_get_200_409(test_client, api_credentials, country_city_room_date):
    ...
    url = f'/api/rooms/{country}/{city}/?room={room_type}&check_in={check_in}&check_out={check_out}'
...
@pytest.mark.parametrize("query_params, error_message", [
    ({"check_in": "2024-03-01", "check_out": "2024-03-05"}, "Missing query parameter: room_type"),    # Missing 'room' query parameter
    ({"room": "single", "check_out": "2024-03-05"}, "Missing query parameter: check_in"),    # Missing 'check_in' query parameter
    ({"room": "single", "check_in": "2024-03-01"}, "Missing query parameter: check_out"),    # Missing 'check_out' query parameter
    ({"room": "single", "check_in": "03-01-2024", "check_out": "03-05-2024"}, "Invalid query parameter value(s)"),    # Invalid date format
    ({"room": "single", "check_in": "2024-03-05", "check_out": "2024-03-01"}, "Incorrect check-in/check-out dates"),    # 'check_out' date before 'check_in' date
])
def test_room_collection_get_400(test_client, api_credentials, query_params, error_message):
    api_key, admin_username = api_credentials
    country = "Finland"
    city = "Oulu"

    url = f'/api/rooms/{country}/{city}/'

    response = test_client.get(url, query_string=query_params, headers={
        "Content-Type": "application/json",
        "Hotels-Api-Key": api_key,
        "Admin-User-Name": admin_username,
    })
...
def test_booking_collection_post(test_client, api_credentials, customer_id, hotel, room_type, check_in, check_out, payment):
    ...
    assert response.status_code in [200, 400, 404], f"Unexpected status code returned: {response.status_code}"
...
def test_booking_item_delete(test_client, api_credentials, booking_ref):
    ...
    assert response.status_code in [200, 403, 404], f"Unexpected status code returned: {response.status_code}"
...
def test_booking_item_put(test_client, api_credentials, booking_ref, customer_id, new_booking_data):
    ...
    assert response.status_code in [200, 403, 404, 400, 409], f"Unexpected status code returned: {response.status_code}"
